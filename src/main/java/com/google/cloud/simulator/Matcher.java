...
class Matcher {
  ...
  List<OrderBookEvent> remove(Order o) {
    Order old = null;
    if (o.getSide() == OrderBookEvent.Side.BUY) {
    ...
    if (old != null) {
      return Arrays.asList(buildEvent(OrderBookEvent.Type.DELETED, o).build());
    } else {
  ...
  List<OrderBookEvent> add(Order o) {
    ...
    // Add new order event
    events.add(buildEvent(OrderBookEvent.Type.NEW, o).build());

    return events;
  }

  private ArrayList<OrderBookEvent> match(Order o) {
    ArrayList<OrderBookEvent> matches = new ArrayList<OrderBookEvent>();
  ...
  private OrderBookEvent.Builder buildEvent(OrderBookEvent.Type type, Order order) {
